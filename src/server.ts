#!/usr/bin/env node
/**
 * Project Cordelia - MCP Memory Server
 *
 * Provides persistent dense memory for Claude via the Model Context Protocol.
 * R1: Read-only L1 hot context retrieval.
 * R2: Write support with optimistic concurrency and audit trail.
 */
...
class CordeliaServer {
  private server: Server;
  private hotContextCache: Map<string, L1HotContext> = new Map();
  private encryptionEnabled = false;
  private integrityInterval: ReturnType<typeof setInterval> | null = null;
  private ttlSweepInterval: ReturnType<typeof setInterval> | null = null;
  ...
  private async loadHotContext(
    userId: string,
    bypassCache = false,
    skipValidation = false
  ): Promise<L1HotContext | null> {
    // Check cache first (unless bypassed)
    if (!bypassCache && this.hotContextCache.has(userId)) {
      return this.hotContextCache.get(userId)!;
    }
    ...
    try {
      ...
      // Skip validation when loading for write operations (e.g., replace)
      // This allows fixing invalid data via a replace operation
      if (skipValidation) {
        return parsed as L1HotContext;
      }

      const validated = L1HotContextSchema.parse(parsed);
      this.hotContextCache.set(userId, validated);
      return validated;
    } catch (error) {
  ...
  private async appendAudit(entries: AuditEntry[]): Promise<void> {
  ...
  private async writeHotContext(
    ...
    if (operation === 'replace') {
      // Full replacement - data must be complete and valid
      ...
      // Patch - deep merge data into current
    ...
    // Write to storage (encrypt if crypto is enabled)
    const cryptoProvider = getDefaultCryptoProvider();
    ...
    // Invalidate cache
    this.hotContextCache.delete(userId);

    // Write audit entries
    const auditEntries: AuditEntry[] = changes
      .filter((c) => c.path !== 'updated_at') // Don't log timestamp changes
      .map((c) => ({
        ts: newUpdatedAt,
        user: userId,
        op: operation,
    ...
    await this.appendAudit(auditEntries);
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (name) {
        ...
        case 'memory_write_hot': {
          ...
          const result = await this.writeHotContext(user_id, operation, data, expected_updated_at);
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  ...
                  version: '0.3.0',
                  layers: {
                    L1_hot: { users, cached: Array.from(this.hotContextCache.keys()) },
                    L2_warm: l2Stats,
        ...
        case 'memory_read_warm': {
          ...
          // Policy check when entity_id is provided
          if (entity_id) {
            const storage = getStorageProvider();
        ...
        case 'memory_write_warm': {
          ...
          // Policy check when entity_id + group_id are provided
        ...
        case 'memory_bind_context': {
          ...
          // Load current context to get existing bindings
          const ctx = await this.loadHotContext(user_id);
          if (!ctx) {
          ...
          const result = await this.writeHotContext(user_id, 'patch', {
  ...
  async run(): Promise<void> {
    ...
    if (integrityIntervalMs > 0) {
      ...
      // Don't keep the process alive just for integrity checks
      this.integrityInterval.unref();
  ...
  private async initEncryption(): Promise<void> {
    ...
    try {
      // Use a global salt for all users (simplifies key management)
      // Per-user salts would require user context at init time
...
const server = new CordeliaServer();
