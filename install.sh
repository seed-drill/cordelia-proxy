...
CORDELIA_CONFIG="$CORDELIA_CONFIG_DIR/config.toml"
mkdir -p "$CORDELIA_CONFIG_DIR"
if [ ! -f "$CORDELIA_CONFIG" ]; then
    cat > "$CORDELIA_CONFIG" << NODEEOF
# Cordelia configuration - single source of truth for identity and paths
...
[paths]
memory_root = "${CORDELIA_CONFIG_DIR}/memory"
...
NODEEOF
    ...
    # Ensure identity section exists in existing config
    if ! grep -q '^\[identity\]' "$CORDELIA_CONFIG" 2>/dev/null; then
        # Prepend identity and paths sections to existing config
        TEMP_CONFIG=$(mktemp)
        cat > "$TEMP_CONFIG" << IDENTEOF
# Cordelia identity - added by install.sh
[identity]
user_id = "${USER_ID}"
...
IDENTEOF
        cat "$CORDELIA_CONFIG" >> "$TEMP_CONFIG"
        mv "$TEMP_CONFIG" "$CORDELIA_CONFIG"
        info "Added identity section to existing config: $CORDELIA_CONFIG"
    else
        info "Config already exists with identity: $CORDELIA_CONFIG"
    fi
fi
...
// No user_id arg needed - hooks read from ~/.cordelia/config.toml
const startHook = {
    matcher: '',
    hooks: [{
        type: 'command',
        command: cordeliaDir + '/hooks/session-start.mjs',
        timeout: 10
    }]
};
...
const endHook = {
    ...
    hooks: [{
        type: 'command',
        command: cordeliaDir + '/hooks/session-end.mjs',
        timeout: 10
    }]
};
